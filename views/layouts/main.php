...
<div class="wrap">
    ...
    if (!Yii::$app->user->isGuest) {
        $items[] = ['label' => 'Занятия', 'url' => ['/lessons']];
        if (Yii::$app->user->identity->role === 'teacher') {
            $items[] = ['label' => 'Группы', 'url' => ['/groups']];
            $items[] = ['label' => 'Студенты', 'url' => ['/students']];
            $items[] = ['label' => 'Предметы', 'url' => ['/predmets']];
            $items[] = ['label' => 'Учителя', 'url' => ['/teachers']];
        }
    }

    $items[] = ['label' => 'Информация', 'url' => ['/site/about']];
